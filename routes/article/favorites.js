...
router.post('/add', authenticate, async (req, res) => {
  ...
  if (!articleId) {
    return res
      .status(400)
      .json({ success: false, message: 'articleId 為必填欄位' })
  }

  try {
    const [articleRows] = await db.query(
      'SELECT 1 FROM article WHERE id = ? LIMIT 1',
      [articleId]
    )
...
router.post('/remove', authenticate, async (req, res) => {
  ...
  try {
    ...
    await db.query(
      'DELETE FROM article_favorites WHERE member_id = ? AND article_id = ?',
      [memberId, articleId]
    )
...
// 查詢會員收藏的所有文章
router.get('/favartical', authenticate, async (req, res) => {
  const memberId = req.member.id

  try {
    const [rows] = await db.query(
      `
      SELECT a.id, a.title, a.article_images, af.created_at
    ...
    console.error('GET /api/article/favorite 錯誤:', err)
    res.status(500).json({ success: false, message: '資料庫錯誤' })
  }
})
